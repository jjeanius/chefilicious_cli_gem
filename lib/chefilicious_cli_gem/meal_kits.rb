class ChefiliciousCliGem::Meal_Kits
  attr_accessor :name, :price, :skill_level, :cooking_time, :allergen, :url, :meal_kits, :meal_type, :cuisine, :food_category, :mealkit
  @@all = []
  ...
  def self.scrape_chef_d
    ...
    meal_kits_chef_d.each do|mealkit_chef_d|
      ...
      binding.pry
      mealkit.description = mealkit_chef_d.css(".from_chef").css("p.description")  #Does not work
